//
// Created by Omeed on 5/15/25.
//
//basically a repeat of Icosphere except specfically for Ball
#include <Solver.h>

void Solver::combineVertices() {
...
void Solver::setSubdivision(int iteration)
{
...
void Solver::setSmooth(bool smooth)
{
...
///////////////////////////////////////////////////////////////////////////////
std::vector<float> Solver::computeIcosahedronVertices()
{
...
void Solver::buildVerticesFlat()
{
    ...
    for(int i = 1; i <= 5; ++i)
    {
        ...
        Solver::computeFaceNormal(v0, v1, v2, n);
        ...
        Solver::computeFaceNormal(v3, v11, v4, n);
...
///////////////////////////////////////////////////////////////////////////////
void Solver::buildVerticesSmooth()
{
    //const float S_STEP = 1 / 11.0f;         // horizontal texture step
    //const float T_STEP = 1 / 3.0f;          // vertical texture step
    const float S_STEP = 186 / 2048.0f;     // horizontal texture step
    ...
    Solver::computeVertexNormal(v, n);
    ...
    scale = Solver::computeScaleForLength(v, 1);
...
// O - S - S - O
///////////////////////////////////////////////////////////////////////////////
void Solver::subdivideVerticesFlat()
{
    if(subdivision <= 1)
        return;
    ...
    for(i = 0; i < indexCount; i += 3)
    {
        ...
        for(j = 1; j <= subdivision; ++j)
        {
            ...
            //     / \newV3/ \       //
            //    v2--*---*---v3     //
            Solver::interpolateVertex(v1, v2, a, radius, newV1);
            Solver::interpolateVertex(v1, v3, a, radius, newV2);
            Solver::interpolateTexCoord(t1, t2, a, newT1);
            Solver::interpolateTexCoord(t1, t3, a, newT2);
            for(k = 0; k <= j; ++k)
            {
                ...
                {
                    a = (float)k / j;
                    Solver::interpolateVertex(newV1, newV2, a, radius, newV3);
                    Solver::interpolateTexCoord(newT1, newT2, a, newT3);
        ...
        for(j = 1; j <= subdivision; ++j)
        {
            for(k = 0; k < j; ++k)
            {
                ...
                Solver::computeFaceNormal(v1, v2, v3, normal);
                ...
                if(k < (j-1))
                {
                    ...
                    v2 = &newVs[i2 * 3];
                    addVertices(v1, v3, v2);
                    Solver::computeFaceNormal(v1, v3, v2, normal);
                    addNormals(normal, normal, normal);
                    t2 = &newTs[i2 * 2];
                    addTexCoords(t1, t3, t2);
                    addIndices(index, index+1, index+2);
...
// O - S - S - O  //
///////////////////////////////////////////////////////////////////////////////
void Solver::subdivideVerticesSmooth()
{
    if(subdivision < 2)
        return;
    ...
    for(i = 0; i < indexCount; i += 3)
    {
        ...
        for(j = 1; j <= subdivision; ++j)
        {
            ...
            {
                a = (float)j / subdivision;    // lerp alpha
                Solver::interpolateVertex(icoV1, icoV2, a, radius, edgeV1);
                Solver::interpolateVertex(icoV1, icoV3, a, radius, edgeV2);
                Solver::interpolateTexCoord(icoT1, icoT2, a, edgeT1);
                Solver::interpolateTexCoord(icoT1, icoT3, a, edgeT2);
            ...
            for(k = 0; k <= j; ++k)
            {
                ...
                else
                {
                    ...
                    a = (float)k / j;
                    Solver::interpolateVertex(edgeV1, edgeV2, a, radius, newV);
                    Solver::interpolateTexCoord(edgeT1, edgeT2, a, newT);
...
///////////////////////////////////////////////////////////////////////////////
void Solver::buildInterleavedVertices()
{
...
///////////////////////////////////////////////////////////////////////////////
void Solver::addVertices(const float v1[3], const float v2[3], const float v3[3])
{
...
///////////////////////////////////////////////////////////////////////////////
void Solver::addNormals(const float n1[3], const float n2[3], const float n3[3])
{
...
///////////////////////////////////////////////////////////////////////////////
void Solver::addTexCoords(const float t1[2], const float t2[2], const float t3[2])
{
...
///////////////////////////////////////////////////////////////////////////////
void Solver::addIndices(unsigned int i1, unsigned int i2, unsigned int i3)
{
...
///////////////////////////////////////////////////////////////////////////////
void Solver::addLineIndices(unsigned int i1, unsigned int i2)
{
...
///////////////////////////////////////////////////////////////////////////////
unsigned int Solver::addSubVertexAttribs(const float v[3], const float n[3], const float t[2])
{
    unsigned int index;     // return value;

    // check if is shared vertex or not first
    if(Solver::isSharedTexCoord(t))
    {
...
///////////////////////////////////////////////////////////////////////////////
void Solver::computeFaceNormal(const float v1[3], const float v2[3], const float v3[3], float n[3])
{
...
///////////////////////////////////////////////////////////////////////////////
void Solver::computeVertexNormal(const float v[3], float normal[3])
{
    // normalize
    float scale = Solver::computeScaleForLength(v, 1);
    normal[0] = v[0] * scale;
    normal[1] = v[1] * scale;
    normal[2] = v[2] * scale;
}
...
///////////////////////////////////////////////////////////////////////////////
float Solver::computeScaleForLength(const float v[3], float length)
{
...
///////////////////////////////////////////////////////////////////////////////
void Solver::interpolateVertex(const float v1[3], const float v2[3], float t, float length, float newV[3])
{
    newV[0] = Solver::lerp(v1[0], v2[0], t);
    newV[1] = Solver::lerp(v1[1], v2[1], t);
    newV[2] = Solver::lerp(v1[2], v2[2], t);
    float scale = Solver::computeScaleForLength(newV, length);
    newV[0] *= scale;
    newV[1] *= scale;
    newV[2] *= scale;
}
...
// return new coord
///////////////////////////////////////////////////////////////////////////////
void Solver::interpolateTexCoord(const float t1[2], const float t2[2], float a, float newT[2])
{
    newT[0] = Solver::lerp(t1[0], t2[0], a);
    newT[1] = Solver::lerp(t1[1], t2[1], a);
}
...
///////////////////////////////////////////////////////////////////////////////
float Solver::lerp(float from, float to, float alpha)
{
...
//      15  16  17  18  19      //
///////////////////////////////////////////////////////////////////////////////
bool Solver::isSharedTexCoord(const float t[2])
{
    ...
    {
        if(Solver::isOnLineSegment(&segments[i], &segments[j], t))
...
///////////////////////////////////////////////////////////////////////////////
bool Solver::isOnLineSegment(const float a[2], const float b[2], const float c[2])
{
...
///////////////////////////////////////////////////////////////////////////////
// generating ball buffers
///////////////////////////////////////////////////////////////////////////////

void Solver::generateLineBuffer() {
    unsigned int EBO;
    unsigned int VBO;
    glGenBuffers(1, &VBO);
    unsigned int VAO;
...
void Solver::generatePointBuffer() {
    unsigned int VBO;
    glGenBuffers(1, &VBO);
    unsigned int VAO;
    glGenVertexArrays(1, &VAO);
...
void Solver::generateSphereBuffer() {
    unsigned int EBO;
    unsigned int VBO;
    glGenBuffers(1, &VBO);
    unsigned int VAO;
